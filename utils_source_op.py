...
class GeneralUtils():
    ...
    def print_messages_SMO(self, mode, train_datas=None, train_labels=None, mutated_datas=None, mutated_labels=None,
                           model=None, mutated_model=None, mutation_ratio=0):
        if mode in ['DR', 'DM']:
            print('Before ' + mode)
    ...
    def print_messages_MMM_generators(self, mode, network=None, test_datas=None, test_labels=None, model=None,
                                      mutated_model=None, STD=0.1, mutation_ratio=0):
...
class ModelUtils():
    ...
    def model_copy(self, model, mode=''):
        ...
        new_model._name = new_model.name + suffix
        return new_model
    ...
    def print_comparision_of_layer_weights(self, old_model, new_model):
        ...
        for index, result in enumerate(booleans_for_layers):
            ...
            print(' {index} | {name} | {result}'.format(index=str(index).rjust(11), name=name.rjust(14),
                                                        result=(not result)))
        print('')
    ...
    def valid_indices_of_mutated_layers_check(self, num_of_layers, indices):
        if indices is not None:
            for index in indices:
                ...
                assert index < num_of_layers, 'Index should not be out of range, where index should be smaller than ' + str(
                    num_of_layers)
                pass
