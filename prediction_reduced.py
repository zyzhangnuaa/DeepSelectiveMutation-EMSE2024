...
# 对约减后的的变异体进行预测  目的是看约减后的变异体上的杀死情况
def prediction():
    ...
    parser.add_argument('-op_list',
                        type=list,
                        default=['NP', 'NEB', 'WS', 'LAa', 'GF'],
                        help='op to be reduced')
    # ['NP', 'NEB', 'WS', 'LAa', 'GF']  for lenet5
    # ['LAa', 'WS', 'NEB', 'NS', 'GF'] for mnist
    # ['LR', 'AFRs', 'DR', 'LAs', 'DF', 'LD', 'LAm', 'NP'] for cifar10
    # ['DR', 'DM', 'LE', 'NP', 'DF', 'LAs'] for svhn
    ...
    for mutant in mutants_path:
        ...
        if mutant_name.split('_')[1] in op_list:
            continue
        mutants_num += 1
    ...
    predictions_path_reduced = os.path.join('predictions_reduced', subject_name)
    if not os.path.exists(predictions_path_reduced):
        try:
            os.makedirs(predictions_path_reduced)
        except OSError as e:
            print('Unable to create folder for analysis results:' + str(e))

    killed_csv_file = os.path.join(predictions_path_reduced, "killed_class.csv")
    unkilled_csv_file = os.path.join(predictions_path_reduced, "unkilled_class.csv")
    mutant_class_input_info_path = os.path.join(predictions_path_reduced, "class_input_info.csv")
    killed_mutant_csv_file = os.path.join(predictions_path_reduced, "killed_mutant.csv")
    unkilled_mutant_csv_file = os.path.join(predictions_path_reduced, "unkilled_mutant.csv")
    killed_input_list_of_mutant_csv_file = os.path.join(predictions_path_reduced, "killed_input_of_mutant.csv")
