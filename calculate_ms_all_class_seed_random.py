...
def calculate_mutation_score(subject_name, test_set_kind, mutated_model_path, predictions_path, op_list, mutantnum):
    ...
    for op in op_relate_killed_class_num.keys():
        if op != 'all':
            ...
            sum += ms
            count += 1
    ms_dic['all'] = round(sum/count, 4)
    print('ms_dic:', ms_dic)

    return op_relate_class_num, op_relate_killed_class_num, mutantnum, ms_dic
...
if __name__ == '__main__':
    ...
    for test_set in ['random0', 'random1', 'random2', 'random3', 'random4']:
        count += 1
        op_relate_class_num, op_relate_killed_class_num, mutantnum, ms_dic = calculate_mutation_score(subject_name,
                                                                               test_set, mutated_model_path,
                                                                               predictions_path, op_list, mutantnum)
        for key in op_relate_killed_class_num.keys():
            ms_dic_aa[key] += ms_dic[key]
            ms_dic_bb[key] += op_relate_killed_class_num[key]

    for key in op_relate_killed_class_num.keys():
        ms_dic_aa[key] = round(ms_dic_aa[key]/5, 4)
        ms_dic_bb[key] = round(ms_dic_bb[key]/(count*200), 4)
    print('ms_dic_aa:', ms_dic_aa)
    # print('ms_dic_bb:', ms_dic_bb)
        # ms_dic = {}
        # sum = 0
        # count = 0
